...
- **Claims aligned to verified live state** (checked 2026-09-13 against `mcp.norric.io/health` and the norric-mcp source): Kreditvakt "Live · ingestion operational" is true again (2,685 companies tracked, pipelines fresh); Sigvik 33,710 BRFs verified; SIGNAL relabelled "Building · MCP tools live" (data activation pending; legacy staging link removed - it showed a US municipal product); MCP endpoint canonicalised to `mcp.norric.io`; API key format unified to `nrk_` (per `issuance/key_gen.py`); Kreditvakt score documented as 0-20 (per `kreditvakt/api.py` `"scale": "0-20"`), docs example fixed.
...
**Decision.** Replaced pre-pivot marketing surface with the May 2026 positioning. Mission updated, indices framework introduced, Pulse banner surfaced. Portfolio rationalised: Jarvis, SiteLoop, Norric Vigil, LeadFlow AI, Betalningskollen all removed from marketing surface (Vigil sensors migrated into Kreditvakt's early-warning tier; the other four killed or out of scope for norric.io). Norric Vattendom added to portfolio at "Building · ingestion in flight" status. SIGNAL retained on its legacy staging domain pending rebrand. Pulse routed to `/pulse` page (not subdomain).
